      ...
      <AreaChart data={data} margin={{ top: 10, right: 12, left: 0, bottom: 0 }}>
        <defs>
          <linearGradient id="growthFill" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="var(--color-go-green)" stopOpacity={0.55} />
            <stop offset="100%" stopColor="var(--color-go-green)" stopOpacity={0.02} />
          </linearGradient>
        </defs>
          ...
          content={({ active, payload, label }) => {
            ...
            return (
              <div className="rounded-xl border border-base bg-elevated px-4 py-3 text-sm shadow-xl">
                <p className="mb-1 font-semibold text-base-fg">{fmtMonth(String(label))}</p>
                <p className="text-go-green">
                  {fmt(d.cumulativeParticipants)} Teilnehmer gesamt
                </p>
                <p className="text-muted">+{fmt(d.newParticipants)} neu · {d.events} Events</p>
              </div>
            );
          }}
        />
